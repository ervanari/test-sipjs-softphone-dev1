...
import { unregisterSIP } from "../lib/sipClient";
...
export default function Home() {
  ...
  const callLogger = useCallLogger(userId);
  ...
  // UI state
  const [activeTab, setActiveTab] = useState<'call-history' | 'calls'>('call-history');
  ...
  // Handle user login
  const handleLogin = (userId: string, userUsername: string) => {
    setIsAuthenticated(true);
    setUserId(userId);
  ...
  const handleRegister = (userId: string, userUsername: string) => {
    setIsAuthenticated(true);
    setUserId(userId);
    setUsername(userUsername);
  };
  ...
  const handleLogout = async () => {
    try {
      // First, unregister from the SIP server
      await unregisterSIP();
      ...
      // Clear SIP data from localStorage
  ...
  const handleIncomingCall = (invitation: any) => {
    ...
    callLogger.logIncomingCall(invitation);
  };
  ...
  const handleOutgoingCall = async (session: any) => {
    ...
    callLogger.logOutgoingCall(session);
        ...
        <div className="min-h-screen bg-[#f0f2f5] flex items-center justify-center p-4">
          <div className="w-full max-w-md">
            <div className="text-center mb-8">
              <h1 className="text-2xl font-bold text-[#128C7E]">WebRTC Softphone</h1>
              <p className="text-gray-600">Please login to access the softphone</p>
            </div>
  ...
  // If user is authenticated but not registered with SIP, show SIP registration
  if (!isRegistered) {
    return (
        <div className="min-h-screen bg-[#f0f2f5] flex items-center justify-center p-4">
          <div className="w-full max-w-md">
            <div className="text-center mb-8">
              <h1 className="text-2xl font-bold text-[#128C7E]">WebRTC Softphone</h1>
              <p className="text-gray-600">Connect to your SIP account to start calling and view call history</p>
            </div>
            <SIPRegistration
  ...
  return (
      <div className="min-h-screen bg-[#f0f2f5] flex flex-col">
          ...
          <div className="flex justify-between items-center">
            ...
            <div className="flex items-center space-x-4">
              ...
              <button
                ...
                className="bg-white text-[#128C7E] px-3 py-1 rounded text-sm font-medium hover:bg-gray-100"
              >
                Logout
              </button>
            </div>
        ...
        <div className="bg-[#128C7E] text-white">
          <div className="flex">
            <button
                className={`flex-1 py-3 text-center font-medium ${activeTab === 'call-history' ? 'border-b-2 border-white' : 'opacity-80'}`}
              ...
              <div className="h-full flex flex-col">
                {inCall ? (
                    ...
                    <div className="flex-1 flex items-center justify-center p-4">
                      {showDialer ? (
                          <div className="w-full max-w-md">
                            <Dialer domain={domain} userId={userId} onCallInitiated={handleOutgoingCall} />
                          </div>
                      ) : (
